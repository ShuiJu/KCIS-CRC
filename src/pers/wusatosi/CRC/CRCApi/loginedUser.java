...
import pers.wusatosi.CRC.CRCApi.UnuseableLoginException.RefusedLoginException.ErrorType;
...
public class loginedUser implements Cloneable,java.io.Serializable{
	...
	public loginedUser(StudentID ID,char[] passwd,StateListener sl) throws IOException,UnuseableLoginException{
		...
		}else throw new UnuseableLoginException.RefusedLoginException(this,backStr);
...
//		server always ignore whether there is a session in the cookie, it will generate another and give it to you, in another way you could
//		say that we don't even need the session...
		cookie=new HashMap<String,String>();
		cookie.put("kcis_rollcall", ID.getValue());
		if (!checkUserServiceability()) throw new UnuseableLoginException.RefusedLoginException(this, ErrorType.UnverifiedUser);
